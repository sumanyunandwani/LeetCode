...
int main() {
	...
	do {
    	...
    	//Computing phi and n
    	n = p*q;}while(n > 4294967297 || n < 456975);
	phi = (p-1) * (q-1);
	...
	//Reading the message
	flag = 1;
	cout<<"Enter the message (only a-z/A-Z, lower case will be converted to upper): ";
	cin.get();
	getline(cin, M);
	...
	for(int i = 0; i < messageLength; i++){
		if(((((int)M[i] < 65)) ) || ( ((int)M[i] > 90) && (((int)M[i] < 97)) ) || (((int)M[i] > 122))) 
			flag = 0;
		else
			flag = 1;
	}
	...
	if(flag == 0){
	    cout<<"Invalid input. Try a-z|A-Z\nExiting program...";
	    exit(1);
	}

	//Text to decimal
	for(int i = messageLength -1, j= 0; i>=0; i--, j++){
		decimal = decimal + ((int)(M[i]) - 64) * pow(26, j);
		cout<<decimal<<endl;
	}
